...
createCRW<-function(tags, tagid=tagid, n.sim = 100, reverse = FALSE){
  ...
  raster_data <- raster("PA_paper/100482_RelativeExtentsLayer/ITEM_REL_mosaic_1987_2015.tif")
...
createbackgroundabsence<-function(tags, tagid=tagid, n.sim = 100){
  ...
  print(tagid)
  tag = tags[which(tags$id==tagid),c('lon', 'lat','dTime')]
  ...
  coordinates(tag.sp) <- ~lon+lat
  coordinates(tags.sp) <- ~lon+lat
  ...
  for (i in 1:dim(tag)[1]){
    ...
    write.csv(data.frame(sim.allbacktags), file=out.alltags.csv, row.names=F)      #OLD (commented out 4ssm)
