...
files <- list.files("output.Predicted")
...
1.  Sampled predictor layers (modification of source layers specified in Table 1 of the manuscript) to obtain values at presences/pseudo-absences in QGIS:

    a)  sample raster tool for grid data
    b)  join attributes by location tool for shapefile (categorical) data (features contain or overlap points; join type one to many)

2.  Created a mask layer from the extent of the Carter et al. dataset ("mask.tif") and clipped predictor layers to this extent for grid predictions
...
#Created proximity rasters in qgis
#Append per matching year using 
